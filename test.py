    ...
    while True:
        ...
        for i in range(len(newmatrix)):
            for j in range(len(newmatrix[0])):
                if not mine[i][j].isnumeric():
                    newstate = curState.copy()
                    if newstate not in exploredSet:
                        heapq.heappush(frontier, (conflict(newstate,newstate[i][j]) + cost + 1, conflict(newstate, n), cost + 1, newstate))
